...
add_amount = st.number_input("Add Amount:", min_value=0.0, step=1.0, key="add", on_change=lambda: update_balance(st.session_state.add, "add"))
subtract_amount = st.number_input("Subtract Amount:", min_value=0.0, step=1.0, key="subtract", on_change=lambda: update_balance(st.session_state.subtract, "subtract"))
